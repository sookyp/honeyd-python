...
def honeyd_event(identifier, payload, gi):
  try:
    dec = ezdict(json.loads(str(payload)))
    tstamp = datetime.datetime.strptime(dec.timestamp, '%Y-%m-%dT%H:%M:%S.%f')
  except:
    print 'exception processing honeyd event'
    traceback.print_exc()
    return

	a_family = get_addr_family(dec.ip_src)
	if a_family == socket.AF_INET:
		geoloc = geoloc_none( gi[a_family].record_by_addr(dec.ip_src) )
		if dec.ip_dst:
			geoloc2 = geoloc_none( gi[a_family].record_by_addr(dec.ip_dst) )
	elif a_family == socket.AF_INET6:
		geoloc = geoloc_none( gi[a_family].record_by_addr_v6(dec.ip_src) )
		if dec.ip_dst:
			geoloc2 = geoloc_none( gi[a_family].record_by_addr(dec.ip_dst) )
